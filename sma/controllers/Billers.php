...
class Billers extends MY_Controller {
    ...
    function add() {
        ...
        $this->form_validation->set_rules('email', $this->lang->line("barcode_prefix"), 'required');
...
//            $config['overwrite'] = true;
            ...
            $data = array('name' => $this->input->post('name'),
                ...
                'country' => $this->input->post('country'),
                'phone' => $this->input->post('phone'),
                'logo' => isset($up) && !empty($up) ? $up['file_name'] : $this->input->post('logo_name'),
                'cf1' => $this->input->post('cf1'),
                'cf2' => $this->input->post('cf2'),
                'cf3' => $this->input->post('cf3'),
                'cf4' => $this->input->post('cf4'),
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('F1', lang('Vat_no'));
                    $this->excel->getActiveSheet()->SetCellValue('G1', lang('Address'));
                    $this->excel->getActiveSheet()->SetCellValue('H1', lang('State'));
                    $this->excel->getActiveSheet()->SetCellValue('I1', lang('Zipcode'));
                    $this->excel->getActiveSheet()->SetCellValue('J1', lang('Country'));
                    $this->excel->getActiveSheet()->SetCellValue('K1', lang('Tax_rate'));
                    $this->excel->getActiveSheet()->SetCellValue('L1', lang('Tax_method'));
                    $this->excel->getActiveSheet()->SetCellValue('M1', lang('invoice_footer'));
                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $customer = $this->site->getCompanyByID($id);
                        $tax = $this->site->getTaxRateByID($customer->tax_rate);
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $customer->vat_no);
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('K' . $row, $tax ? $tax->name : "");
                        $this->excel->getActiveSheet()->SetCellValue('L' . $row, $customer->tax_method == 0 ? "Inclusive" : "Exclusive");
                        $this->excel->getActiveSheet()->SetCellValue('M' . $row, $customer->invoice_footer);
                        $row++;
                    }
