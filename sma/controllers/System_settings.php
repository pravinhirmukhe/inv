...
class system_settings extends MY_Controller {
    ...
    public function setStatus() {
        ...
        $id = $_GET['id'];
        $table = $_GET['tab'];
        $r = $this->site->setStatus($key, $id, $table);
        if ($r) {
...
//        $this->form_validation->set_rules('barcode_prefix', lang('barcode_prefix'), 'trim');
...
//                'barcode_prefix' => $this->input->post('barcode_prefix'),
...
//    }
    ...
    function permissions($id = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'products-challan' => $this->input->post('products-challan')
            );
    ...
    /**
     * Name: Products Parametes
     * Setting for products parameters.
     */
    function products_para() {

        $this->data['erro'] = $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['p'] = $this->site->getAttributes();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('products_para')));
        $meta = array('page_title' => lang('products_para'), 'bc' => $bc);
        $this->page_construct('settings/product_para', $meta, $this->data);
    }

    public function getProduct_para($tab) {
        $this->load->library('datatables');
        $edit_link = anchor('system_settings/EditProduct_para/$2/$1', '<i class="fa fa-pencil"></i> ' . 'Edit', 'data-toggle="modal" data-target="#myModal"');
        $delete_link = '<a href="javascript:;" onclick="deletePara(\'$2\',$1)"> <i class="fa fa-trash"></i>Delete</a>';
        ...
        $tab1 = $this->db->dbprefix($tab);
        if ($tab == "department") {
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$com}.name as store,{$tab1}.status");
            $this->datatables->join($com, "{$com}.id={$tab1}.store_id", "left");
        } else if ($tab == "section") {
            $t = $this->db->dbprefix('department');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$t}.name as names,{$tab1}.status");
            $this->datatables->join($t, "{$t}.id={$tab1}.department_id", "left");
        } else if ($tab == "product_items") {
            $t = $this->db->dbprefix('section');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$t}.name as names,{$tab1}.status");
            $this->datatables->join($t, "{$t}.id={$tab1}.section_id", "left");
        } else if ($tab == "type") {
            $t = $this->db->dbprefix('product_items');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$t}.name as names,{$tab1}.status");
            $this->datatables->join($t, "{$t}.id={$tab1}.product_items_id", "left");
        } else if ($tab == "brands") {
            $t = $this->db->dbprefix('type');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$t}.name as names,{$tab1}.status");
            $this->datatables->join($t, "{$t}.id={$tab1}.type_id", "left");
        } else if ($tab == "design") {
            $t = $this->db->dbprefix('brands');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$t}.name as names,{$tab1}.status");
            $this->datatables->join($t, "{$t}.id={$tab1}.brands_id", "left");
        } else if ($tab == "style") {
            $t = $this->db->dbprefix('design');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$t}.name as names,{$tab1}.status");
            $this->datatables->join($t, "{$t}.id={$tab1}.design_id", "left");
        } else if ($tab == "pattern") {
            $t = $this->db->dbprefix('style');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$t}.name as names,{$tab1}.status");
            $this->datatables->join($t, "{$t}.id={$tab1}.style_id", "left");
        } else if ($tab == "fitting") {
            $t = $this->db->dbprefix('pattern');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$t}.name as names,{$tab1}.status");
            $this->datatables->join($t, "{$t}.id={$tab1}.pattern_id", "left");
        } else if ($tab == "fabric") {
            $t = $this->db->dbprefix('fitting');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$t}.name as names,{$tab1}.status");
            $this->datatables->join($t, "{$t}.id={$tab1}.fitting_id", "left");
        } else if ($tab == "size") {
            $dt = $this->db->dbprefix('department');
            $st = $this->db->dbprefix('section');
            $pt = $this->db->dbprefix('product_items');
            $this->datatables->select("{$tab1}.id,{$tab1}.name,{$tab1}.code,{$dt}.name as dept,{$st}.name as section,{$pt}.name as prod,{$tab1}.status");
            $this->datatables->join($dt, "{$dt}.id={$tab1}.department_id", "left");
            $this->datatables->join($st, "{$st}.id={$tab1}.section_id", "left");
            $this->datatables->join($pt, "{$pt}.id={$tab1}.product_items_id", "left");
        } else {
            $this->datatables->select("id,name,status");
        }
        $this->datatables->from($tab1);
        $this->datatables->add_column("Actions", $action, "{$tab1}.id,{$tab}");
        echo $this->datatables->generate();
//        $this->db->get();
//        echo "<pre>";
...
//        die();
    ...
    public function updateAttribute() {
        ...
        $rs = $this->db->update('attributes', $arr, array('id' => 1));
        if ($rs) {
            $this->session->set_flashdata('message', lang("update_attribute"));
            redirect("system_settings/products_para");
        } else {
            $this->session->set_flashdata('error', lang("update_failed_attribute"));
            redirect("system_settings/products_para");
        }
    }

    public function AddProduct_para($tab) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
//        $this->data['angular_modal_js'] = $this->site->angular_modal_js();
        $this->data['tab'] = $tab;
        $this->load->view($this->theme . 'settings/addPara', $this->data);
    }

    public function EditProduct_para($tab, $id) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['tab'] = $tab;
        $this->data['d'] = $this->db->get_where($tab, array('id' => $id))->row();
        $this->load->view($this->theme . 'settings/editPara', $this->data);
    }

    public function deletePara($tab, $id) {
        $x = $this->db->delete($tab, array('id' => $id));
        if ($x) {
    ...
    public function AddAjaxProduct_para($tab) {
        $data = $this->input->get();
        $data['name'] = $this->input->get('para_name');
        $data['status'] = $this->input->get('status');
        $data['create_date'] = date('Y-m-d h:i:s');
        $data['userby'] = $this->session->userdata('user_id');
        unset($data['v']);
        unset($data['para_name']);
        if ($tab == "size") {
            $data['code'] = $this->input->get('code');
        }
        $x = $this->db->insert($tab, $data);
        if ($x) {
            echo json_encode(array('s' => "true"));
        } else {
            echo json_encode(array('s' => "false"));
    ...
    public function EditAjaxProduct_para($tab) {
        $data = $this->input->get();
        $data['name'] = $this->input->get('para_name');
        $data['status'] = $this->input->get('status');
        $data['create_date'] = date('Y-m-d h:i:s');
        ...
        $x = $this->db->update($tab, $data, array('id' => $this->input->get('id')));
        if ($x) {
            echo json_encode(array('s' => "true"));
        } else {
            echo json_encode(array('s' => "false"));
    ...
    /**
     * Name: Products Parametes
     * Setting for products discounts.
     */
    function products_discounts() {

        $this->data['erro'] = $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['p'] = $this->site->getAttributes();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('products_discount')));
        $meta = array('page_title' => lang('products_discount'), 'bc' => $bc);
        $this->page_construct('settings/products_discounts', $meta, $this->data);
    }

    public function getProduct_margin() {
        $this->load->library('datatables');
        $edit_link = anchor('system_settings/EditProduct_margin/$1', '<i class="fa fa-pencil"></i> ' . 'Edit', 'data-toggle="modal" data-target="#myModal"');
        $delete_link = '<a href="javascript:;" onclick="deleteMargin(\'$2\',$1)"> <i class="fa fa-trash"></i>Delete</a>';
        $action = '<div class="text-center"><div class="btn-group text-left">'
                . '<button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'
                . 'Actions' . ' <i style="color:#fff" class="fa fa-caret-down"></i></button>
		<ul class="dropdown-menu pull-right" role="menu">
        ...
        $prod_item = $this->db->dbprefix('product_items');
        $type = $this->db->dbprefix('type');
        $brands = $this->db->dbprefix('brands');
        $this->datatables->select("{$margin}.id,{$com}.name as store,{$dept}.name as dept,{$sec}.name as section,{$prod_item}.name as prod_item,{$type}.name as type,{$brands}.name as brands,{$margin}.margin,{$margin}.status");
        $this->datatables->join("companies", "{$margin}.store_id=companies.id", 'left');
        $this->datatables->join("department", "{$margin}.department_id=department.id", 'left');
        $this->datatables->join("section", "{$margin}.section_id=section.id", 'left');
        $this->datatables->join("product_items", "{$margin}.product_item_id=product_items.id", 'left');
        $this->datatables->join("type", "{$margin}.type_id=type.id", 'left');
        $this->datatables->join("brands", "{$margin}.brands_id=brands.id", 'left');
        $this->datatables->from($margin);
        $this->datatables->add_column("Actions", $action, "{$margin}.id,product_margin");
        echo $this->datatables->generate();
    }

    public function AddProduct_margin($tab) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['angular_modal_js'] = $this->site->angular_modal_js();
        $this->data['tab'] = $tab;
        $this->load->view($this->theme . 'settings/addproduct_margin', $this->data);
    }

    public function AddAjaxProduct_margin($tab) {
        $data = $this->input->get();
        $data['status'] = $this->input->get('status');
        $data['create_date'] = date('Y-m-d h:i:s');
        $data['userby'] = $this->session->userdata('user_id');
        ...
        $x = $this->db->insert("product_margin", $data);
        if ($x) {
            echo json_encode(array('s' => "true"));
        } else {
            echo json_encode(array('s' => "false"));
    ...
    public function EditProduct_margin($id) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['margin'] = $this->db->get_where("product_margin", array('id' => $id))->row();
        $this->load->view($this->theme . 'settings/editproduct_margin', $this->data);
    }

    public function EditAjaxProduct_margin($tab) {
        $data = $this->input->get();
        unset($data['v']);
        unset($data['id']);
        if ($tab == "size") {
        ...
        $x = $this->db->update("product_margin", $data, array('id' => $this->input->get('id')));
        if ($x) {
            echo json_encode(array('s' => "true"));
        } else {
            echo json_encode(array('s' => "false"));
    ...
    public function getProduct_discount() {
        $this->load->library('datatables');
        $edit_link = anchor('system_settings/EditProduct_discount/$1', '<i class="fa fa-pencil"></i> ' . 'Edit', 'data-toggle="modal" data-target="#myModal"');
        $delete_link = '<a href="javascript:;" onclick="deleteDiscount(\'$2\',$1)"> <i class="fa fa-trash"></i>Delete</a>';
        $action = '<div class="text-center"><div class="btn-group text-left">'
                . '<button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'
                . 'Actions' . ' <i style="color:#fff" class="fa fa-caret-down"></i></button>
		<ul class="dropdown-menu pull-right" role="menu">
        ...
        $margin = $this->db->dbprefix('product_discount');
        $com = $this->db->dbprefix('companies');
        $dept = $this->db->dbprefix('department');
        $sec = $this->db->dbprefix('section');
        $prod_item = $this->db->dbprefix('product_items');
        $type = $this->db->dbprefix('type');
        $brands = $this->db->dbprefix('brands');
        $this->datatables->select("{$margin}.id,{$com}.name as store,{$dept}.name as dept,{$sec}.name as section,{$prod_item}.name as prod_item,{$type}.name as type,{$brands}.name as brands,{$margin}.discount,{$margin}.status");
        $this->datatables->join("companies", "{$margin}.store_id=companies.id", 'left');
        $this->datatables->join("department", "{$margin}.department_id=department.id", 'left');
        $this->datatables->join("section", "{$margin}.section_id=section.id", 'left');
        $this->datatables->join("product_items", "{$margin}.product_item_id=product_items.id", 'left');
        ...
        $this->datatables->join("brands", "{$margin}.brands_id=brands.id", 'left');
        $this->datatables->from($margin);
        $this->datatables->add_column("Actions", $action, "{$margin}.id,product_discount");
        echo $this->datatables->generate();
    }

    public function AddProduct_discount($tab) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['angular_modal_js'] = $this->site->angular_modal_js();
        $this->data['tab'] = $tab;
        $this->load->view($this->theme . 'settings/addproduct_discount', $this->data);
    }

    public function AddAjaxProduct_discount($tab) {
        $data = $this->input->get();
        $data['status'] = $this->input->get('status');
        $data['create_date'] = date('Y-m-d h:i:s');
        $data['userby'] = $this->session->userdata('user_id');
        ...
        $x = $this->db->insert("product_discount", $data);
//        echo "<pre>";
//        print_r($this->db->last_query());
//        echo "</pre>";
//        die();
    ...
    public function EditProduct_discount($id) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['margin'] = $this->db->get_where("product_discount", array('id' => $id))->row();
        $this->load->view($this->theme . 'settings/editproduct_discount', $this->data);
    }

    public function EditAjaxProduct_discount($tab) {
        $data = $this->input->get();
        unset($data['v']);
        unset($data['id']);
        if ($tab == "size") {
        ...
        $x = $this->db->update("product_discount", $data, array('id' => $this->input->get('id')));
        if ($x) {
            echo json_encode(array('s' => "true"));
        } else {
            echo json_encode(array('s' => "false"));
    ...
    public function getProduct_spe_discount() {
        $this->load->library('datatables');
        $edit_link = anchor('system_settings/EditProduct_spe_discount/$1', '<i class="fa fa-pencil"></i> ' . 'Edit', 'data-toggle="modal" data-target="#myModal"');
        $delete_link = '<a href="javascript:;" onclick="deleteDiscount(\'$2\',$1)"> <i class="fa fa-trash"></i>Delete</a>';
        $action = '<div class="text-center"><div class="btn-group text-left">'
                . '<button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'
                . 'Actions' . ' <i style="color:#fff" class="fa fa-caret-down"></i></button>
        ...
        $this->datatables->select("id, product_code, product_name, mrp, purchase_rate, margin, old_discount, special_discount, status");
        $this->datatables->from("product_special_discount");
        $this->datatables->add_column("Actions", $action, "id,product_special_discount");
        echo $this->datatables->generate();
    }

    public function getProduct_sea_discount() {
        $this->load->library('datatables');
        $edit_link = anchor('system_settings/EditProduct_spe_discount/$1', '<i class="fa fa-pencil"></i> ' . 'Edit', 'data-toggle="modal" data-target="#myModal"');
        $delete_link = '<a href="javascript:;" onclick="deleteDiscount(\'$2\',$1)"> <i class="fa fa-trash"></i>Delete</a>';
        $action = '<div class="text-center"><div class="btn-group text-left">'
        ...
        $margin = $this->db->dbprefix('product_seasonal_discount');
        $com = $this->db->dbprefix('companies');
        $dept = $this->db->dbprefix('department');
        $sec = $this->db->dbprefix('section');
        $prod_item = $this->db->dbprefix('product_items');
        $type = $this->db->dbprefix('type');
        $brands = $this->db->dbprefix('brands');
        $prod_d = $this->db->dbprefix('product_discount');
        $prod_m = $this->db->dbprefix('product_margin');
        $this->datatables->select("{$margin}.id,{$com}.name as store,{$dept}.name as dept,{$sec}.name as section,{$prod_item}.name as prod_item,{$type}.name as type,{$brands}.name as brands,{$margin}.margin,{$margin}.reg_discount,{$margin}.extra_discount,{$margin}.status");
        $this->datatables->join("companies", "{$margin}.store_id=companies.id", 'left');
        $this->datatables->join("department", "{$margin}.department_id=department.id", 'left');
        $this->datatables->join("section", "{$margin}.section_id=section.id", 'left');
        $this->datatables->join("product_items", "{$margin}.product_item_id=product_items.id", 'left');
        $this->datatables->join("type", "{$margin}.type_id=type.id", 'left');
        $this->datatables->join("brands", "{$margin}.brands_id=brands.id", 'left');
//        $this->datatables->join("product_margin", "$margin.store_id = $prod_m.store_id AND $margin.type_id = $prod_m.type_id AND $margin.section_id = $prod_m.section_id AND $margin.product_item_id = $prod_m.product_item_id AND $margin.department_id = $prod_m.department_id AND $margin.brands_id = $prod_m.brands_id", 'left');
//        $this->datatables->join("product_discount", "$margin.brands_id = $prod_d.brands_id AND $margin.department_id = $prod_d.department_id AND $margin.section_id = $prod_d.section_id AND $margin.product_item_id = $prod_d.product_item_id AND $margin.store_id = $prod_d.store_id AND $margin.type_id = $prod_d.type_id", 'left');
        $this->datatables->from($margin);
        $this->datatables->add_column("Actions", $action, "{$margin}.id,{$margin}");
        echo $this->datatables->generate();
    }

    public function AddProduct_spe_discount($tab) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['angular_modal_js'] = $this->site->angular_modal_js();
        $this->data['tab'] = $tab;
        $this->load->view($this->theme . 'settings/addproduct_spe_discount', $this->data);
    }

    public function saveProd_Spe_Dis() {
        $data = $this->input->post();
        $data['create_date'] = date('Y-m-d h:i:s');
        $data['userby'] = $this->session->userdata('user_id');
        unset($data['addPara']);
        $x = $this->db->insert("product_special_discount", $data);
        if ($x) {
            $this->session->set_flashdata('message', lang("prod_spe_dis_added"));
            redirect("system_settings/products_discounts");
        } else {
            $this->session->set_flashdata('error', lang("prod_spe_dis_failed"));
            redirect("system_settings/products_discounts");
        }
    }

    public function EditProduct_spe_discount($id) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['spe'] = $this->db->get_where("product_special_discount", array('id' => $id))->row();
        $this->load->view($this->theme . 'settings/editproduct_spe_discount', $this->data);
    }

    public function editProd_Spe_Dis($id) {
        $data = $this->input->post();
        unset($data['addPara']);
        $x = $this->db->update("product_special_discount", $data, array('id' => $id));
        if ($x) {
            $this->session->set_flashdata('message', lang("prod_spe_dis_updated"));
            redirect("system_settings/products_discounts");
        } else {
            $this->session->set_flashdata('error', lang("prod_spe_dis_updating_failed"));
            redirect("system_settings/products_discounts");
        }
    }

    public function deletediscount($tab, $id) {
        $x = $this->db->delete($tab, array('id' => $id));
        if ($x) {
            echo json_encode(array('s' => "true"));
        } else {
    ...
    /**
     * Name: Minimum Quantity Level
     * Setting for products parameters.
     */
    function min_qty_level() {
        $this->data['error'] = $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['p'] = $this->site->getAttributes();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('min_qty_level')));
        $meta = array('page_title' => lang('min_qty_level'), 'bc' => $bc);
        $this->page_construct('settings/min_qty_lvl', $meta, $this->data);
    }

    public function getMin_qty_lvl() {
        $this->load->library('datatables');
        $edit_link = anchor('system_settings/Editmin_qty_lvl/$2/$1', '<i class="fa fa-pencil"></i> ' . 'Edit', 'data-toggle="modal" data-target="#myModal"');
        $delete_link = '<a href="javascript:;" onclick="deleteMin_qty_lvl(\'$2\',$1)"> <i class="fa fa-trash"></i>Delete</a>';
        $action = '<div class="text-center"><div class="btn-group text-left">'
                . '<button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'
                . 'Actions' . ' <i style="color:#fff" class="fa fa-caret-down"></i></button>
		<ul class="dropdown-menu pull-right" role="menu">
        ...
        $margin = $this->db->dbprefix('min_qty_lvl');
        $com = $this->db->dbprefix('companies');
        $dept = $this->db->dbprefix('department');
        $sec = $this->db->dbprefix('section');
        $prod_item = $this->db->dbprefix('product_items');
        ...
        $this->datatables->select("{$margin}.id,{$com}.name as store,"
                . "{$dept}.name as dept,{$sec}.name as section,"
                . "{$prod_item}.name as prod_item,{$type}.name as type,"
                . "{$brands}.name as brands,{$design}.name as design,"
                . "{$color}.name as color,{$size}.name as size,"
                . "{$margin}.status");
        $this->datatables->join("companies", "{$margin}.store_id=companies.id", 'left');
        $this->datatables->join("department", "{$margin}.department=department.id", 'left');
        $this->datatables->join("section", "{$margin}.section=section.id", 'left');
        $this->datatables->join("product_items", "{$margin}.product_items=product_items.id", 'left');
        $this->datatables->join("type", "{$margin}.type_id=type.id", 'left');
        $this->datatables->join("brands", "{$margin}.brands=brands.id", 'left');
        $this->datatables->join("design", "{$margin}.design=design.id", 'left');
        $this->datatables->join("color", "{$margin}.color=color.id", 'left');
        $this->datatables->join("size", "{$margin}.size=size.id", 'left');
        $this->datatables->from($margin);
        $this->datatables->add_column("Actions", $action, "$margin.id,$margin");
        echo $this->datatables->generate();
//        $this->db->get();
...
//    }
    ...
    public function AddMin_qty_lvl($tab) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
//        $this->data['angular_modal_js'] = $this->site->angular_modal_js();
        $this->data['tab'] = $tab;
        $this->load->view($this->theme . 'settings/addMin_qty_lvl', $this->data);
    }

//
    public function Editmin_qty_lvl($tab, $id) {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['min'] = $this->db->get_where("min_qty_lvl", array('id' => $id))->row();
        $this->load->view($this->theme . 'settings/editmin_qty_lvl', $this->data);
    }

    public function SaveMin_qty_lvl() {
        $data = $this->input->post();
        $data['status'] = "Active";
        $data['create_date'] = date('Y-m-d h:i:s');
        $data['userby'] = $this->session->userdata('user_id');
        unset($data['add_user']);
        $x = $this->db->insert("min_qty_lvl", $data);
        if ($x) {
            $this->session->set_flashdata('message', lang("min_qty_lvl_added"));
            redirect("system_settings/min_qty_level");
        } else {
            $this->session->set_flashdata('error', lang("min_qty_lvl_failed"));
            redirect("system_settings/min_qty_level");
        }
    }

//
    public function updateMin_qty_lvl($id) {
        $data = $this->input->post();
        $x = $this->db->update("min_qty_lvl", $data, array('id' => $id));
        if ($x) {
            $this->session->set_flashdata('message', lang("min_qty_lvl_updated"));
            redirect("system_settings/min_qty_level");
        } else {
            $this->session->set_flashdata('error', lang("min_qty_lvl_updating_failed"));
            redirect("system_settings/min_qty_level");
        }
    }
