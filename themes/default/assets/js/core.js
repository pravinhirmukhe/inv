...
function changeStatus(k, i, t) {

    var chStatus = {key: k, id: i, tab: t};

    $.ajax({
        url: site.base_url + "system_settings/setStatus",
        data: chStatus,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
...
function formatcolor(x) {
    $c = JSON.parse(x);
    $color = "";
    if ($c.colortype == "Assorted") {
        $.each(color, function (k, v) {
            if ($.inArray(v.id, $c.colorassorted)) {
//                $color
            }
        });
    }
