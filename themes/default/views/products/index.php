<script>
    var color = <?= json_decode($color) ?>;
            var size = <?= json_decode($size) ?>;</script>
...
<script>
    ...
    ?> {"mRender": formatQuantity}, null, <?php
    if (!$warehouse_id || !$Settings->racks) {
        echo '{"bVisible": false},';
    } else {
        echo '{"bSortable": true},';
    }
    ?> null, null, {"mRender": null}, {"mRender": null}, {"bSortable": false}
            ]
...
$col = 3;
if ($Owner || $Admin) {
    echo '{column_number : 4, filter_default_label: "[' . lang('product_cost') . ']", filter_type: "text", data: [] },';
    echo '{column_number : 5, filter_default_label: "[' . lang('product_price') . ']", filter_type: "text", data: [] },';
    $col += 2;
} else {
    if ($this->session->userdata('show_cost')) {
        $col++;
...
$col++;
echo $col;
?>, filter_default_label: "[<?= lang('prod_type'); ?>]", filter_type: "text", data: []},
<?php
//if ($warehouse_id && $Settings->racks) {
$col++;
echo '{column_number : ' . $col . ', filter_default_label: "[' . lang('dept') . ']", filter_type: "text", data: [] },';
//}
?>
    {column_number: <?php
$col++;
echo $col;
?>, filter_default_label: "[<?= lang('prod'); ?>]", filter_type: "text", data: []},
    ], "footer");
    });
</script>
<?php
