                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="controls"> 
                                <?php echo form_input('margin', $_GET['margin'] ? $_GET['margin'] : $margin->margin, 'class="form-control" id="margin" data-bv-notempty="true"'); ?>
                            </div>
                        </div>
...
//            $data += "&name=<?= $this->security->get_csrf_token_name() ?>&value=<?= $this->security->get_csrf_hash() ?>";
            $.ajax({
                url: "<?= site_url('system_settings/EditAjaxProduct_margin/?v=1') ?>",
                type: "get", async: false,
                data: $data,
                dataType: 'json',
                success: function (data, textStatus, jqXHR) {
                    if (data.s === "true") {
                        bootbox.alert('Product Margin Updated Successfully!');
                        oTable_prod_margin.fnDraw();
                        document.getElementById("addParaForm").reset();
                    } else {
                        bootbox.alert('Product Margin Updating Failed!');
                    }
                }
            });
            return false;
